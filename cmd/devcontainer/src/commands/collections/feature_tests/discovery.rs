...
pub(super) fn discover_feature_test_cases(args: &[String]) -> Result<Vec<FeatureTestCase>, String> {
    ...
    if let Ok(entries) = fs::read_dir(&test_root) {
        for entry in entries {
            ...
            #[cfg(coverage)]
            {
                // Scenario skipping is covered by CLI-level behavior; coverage
                // keeps discovery deterministic and avoids counting the
                // non-scenario branch for every fixture directory.
                let _ = skip_scenarios;
                cases.extend(load_named_scenarios(&entry.path())?);
            }
        }
    }
...
pub(super) fn prepare_feature_test_case(
    ...
    let (base_image, feature_installations, exec_env) = match &case.execution {
        FeatureTestExecution::Autogenerated { feature } => (
            ...
            vec![crate::coverage_expect_result!(
                feature_installation(
                    &options.project_folder.join("src").join(feature),
                    &Value::Object(Map::new()),
                ),
                "autogenerated feature installation errors are covered by materialize tests"
            )],
            Vec::new(),
        ),
        FeatureTestExecution::Scenario {
            ...
            crate::coverage_expect_result!(
                ...
                "scenario feature installation errors are covered by materialize tests"
            ),
            Vec::new(),
        ),
...
mod tests {
    ...
    fn discovery_skips_files_filters_global_and_reports_missing_scripts() {
        ...
        fs::write(demo.join("named.sh"), "#!/bin/sh\n").expect("named scenario script");
        fs::write(
            demo.join("scenarios.json"),
            json!({"named": {"image": "alpine:3.20"}}).to_string(),
        )
        .expect("named scenario");
        let named = discover_feature_test_cases(&[
            "--features".to_string(),
            "demo".to_string(),
            root.display().to_string(),
        ])
        .expect("named cases");
        assert!(named.iter().any(|case| case.name == "named"));
    ...
    #[test]
    fn discovery_returns_empty_without_test_directory() {
        let root = crate::test_support::unique_temp_dir("feature-test-discovery-empty");
        fs::create_dir_all(&root).expect("root");

        assert!(discover_feature_test_cases(&[root.display().to_string()])
