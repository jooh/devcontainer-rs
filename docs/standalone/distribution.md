...
Release automation renders `Formula/devcontainer-rs.rb` after GitHub Release assets are published, then commits and pushes the formula update to the tap repository. The workflow requires a `HOMEBREW_TAP_TOKEN` secret with write access to `jooh/homebrew-devcontainer-rs`.
